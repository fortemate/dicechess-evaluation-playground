...
- **Containerized `cloudflared` cannot reach the playground:** confirm `cloudflared` and the playground share the `edge` network, `cloudflared` is not attached to `evaluation`, the Tunnel origin uses `http://playground:3000`, and the connector's persisted Compose configuration includes that external network. Do not route the connector to its own `127.0.0.1`; preserve any pre-existing connector networks required by unrelated routes.
